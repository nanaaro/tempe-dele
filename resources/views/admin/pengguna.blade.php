...
<div class="flex items-center gap-3 max-w-7xl mx-auto px-8 my-5">
    ...
    {{-- tombol upload --}}
        <button type="button" title="Upload Presensi" onclick="openCreateModal()"
            class="flex h-10 w-10 items-center justify-center rounded-full bg-[#faa938] text-white hover:brightness-95 transition-all">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4.75 w-4.75" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<div id="alertBox" class="hidden max-w-7xl mx-auto px-8 mb-3">
    ...
    <table class="min-w-full rounded-xl">
        <thead>
            <tr class="bg-gray-50">
                ...
                <th class="p-5 text-center text-sm font-semibold text-gray-900">NIP BPS</th>
                <th class="p-5 text-center text-sm font-semibold text-gray-900">Email</th>
                <th class="p-5 text-center text-sm font-semibold text-gray-900">Password</th>
                <th class="p-5 text-center text-sm font-semibold text-gray-900">Golongan</th>
                <th class="p-5 text-center text-sm font-semibold text-gray-900">Role</th>
                <th class="p-5 text-center text-sm font-semibold text-gray-900 rounded-tr-xl">Aksi</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-300">
            @forelse ($pegawai as $p)
            <tr class="bg-white hover:bg-gray-50 text-center" data-id="{{ $p->id_pegawai }}">

                {{-- Nama --}}
                <td class="p-5 text-sm text-gray-900 text-left">
                    <span class="cell-display">{{ $p->nama ?? '—' }}</span>
                ...
                <td class="p-5 text-sm text-gray-900">
                    <span class="cell-display">{{ $p->nip ?? '—' }}</span>
                ...
                {{-- NIP Lama --}}
                <td class="p-5 text-sm text-gray-900">
                    <span class="cell-display">{{ $p->nip_lama ?? '—' }}</span>
                    <input type="text" name="nip_lama" value="{{ $p->nip_lama }}"
                        class="cell-edit hidden w-full border border-gray-200 rounded-lg px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none"/>
                </td>

                {{-- Email --}}
                <td class="p-5 text-sm text-gray-900">
                    <span class="cell-display">{{ $p->email ?? '—' }}</span>
                ...
                <td class="p-5 text-sm text-gray-900">
                    ...
                    <button type="button"
                        class="cell-edit hidden text-xs text-[#faa938] hover:underline"
                        onclick="openPasswordModal({{ $p->id_pegawai }}, '{{ $p->nama }}')">
                        Ganti password
                    </button>
                </td>
                ...
                {{-- Golongan --}}
                <td class="p-5 text-sm text-gray-900">
                    <span class="cell-display">{{ $p->golongan ?? '—' }}</span>
                ...
                <td class="p-5 text-sm text-gray-900">
                    <div class="flex items-center justify-center gap-1">
                        ...
                        <button class="btn-save hidden p-2 rounded-full hover:bg-green-50 transition-all flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                            </svg>
                        </button>
                        <button class="btn-cancel hidden p-2 rounded-full hover:bg-red-50 transition-all flex items-center">
            ...
            @empty
            <tr>
                <td colspan="8" class="p-10 text-center text-sm text-gray-400">Belum ada data pegawai.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
<div class="flex justify-center mt-6">
    <nav class="inline-flex items-center p-1 rounded bg-white space-x-2">
        ...
        @else
            ...
            href="{{ $pegawai->previousPageUrl() }}&search={{ request('search') }}">
                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z"/>
                </svg>
            </a>
        ...
        <p class="text-gray-500 text-sm">Page {{ $pegawai->currentPage() }} of {{ $pegawai->lastPage() }}</p>

        @if ($pegawai->hasMorePages())
            <a class="p-1 rounded border text-black bg-white hover:text-white hover:bg-[#faa938] hover:border-[#faa938]"
            href="{{ $pegawai->nextPageUrl() }}&search={{ request('search') }}">
                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
...
<div id="modalCreatePegawai" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">

        <div class="flex items-center justify-between mb-6">
            <h2 class="text-sm font-semibold text-gray-900">Tambah Pegawai</h2>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2">
                    <label class="block text-xs font-medium text-gray-500 mb-1">Nama Pegawai</label>
                    <input type="text" id="createNama" placeholder="Nama lengkap"
                        class="w-full h-10 rounded-xl border border-gray-200 px-4 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                </div>
                ...
                {{-- NIP BPS --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">NIP BPS</label>
                    <input type="text" id="createNipBps" placeholder="NIP BPS"
                        class="w-full h-10 rounded-xl border border-gray-200 px-4 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <input type="email" id="createEmail" placeholder="email@bps.go.id"
                        class="w-full h-10 rounded-xl border border-gray-200 px-4 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                </div>

                {{-- Golongan --}}
                <div>
                    <label class="block text-xs font-medium text-gray-500 mb-1">Golongan</label>
                    <select id="createGolongan"
                        class="w-full h-10 rounded-xl border border-gray-200 px-4 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" id="createPassword" placeholder="Password"
                            class="w-full h-10 rounded-xl border border-gray-200 pl-4 pr-10 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        <button type="button" onclick="toggleVisibility('createPassword', 'eyeCreate1')"
                            class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg id="eyeCreate1" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" id="createPasswordKonfirmasi" placeholder="Ulangi password"
                            class="w-full h-10 rounded-xl border border-gray-200 pl-4 pr-10 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        <button type="button" onclick="toggleVisibility('createPasswordKonfirmasi', 'eyeCreate2')"
                            class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg id="eyeCreate2" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                        </button>
                    </div>
                    <p id="createPasswordError" class="hidden mt-1 text-xs text-red-500">Password tidak cocok</p>
                </div>
        ...
        <div class="flex justify-end gap-2 mt-6">
            <button type="button" onclick="closeCreateModal()"
                class="px-4 py-2 text-sm font-medium rounded-xl border border-gray-200 text-gray-600 hover:border-gray-300 transition-colors">
                Batal
            </button>
            <button type="button" onclick="submitCreatePegawai()"
                class="px-4 py-2 text-sm font-medium rounded-xl bg-[#faa938] text-white hover:brightness-95 transition-all">
                Simpan
            </button>
        </div>
...
<div id="modalPassword" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">

        <div class="flex items-center justify-between mb-6">
            <h2 class="text-sm font-semibold text-gray-900">Ganti Password</h2>
        ...
        <p class="text-xs text-gray-500 mb-4">
            Pegawai: <span id="passwordModalNama" class="font-medium text-gray-800"></span>
        </p>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" id="inputPasswordBaru" placeholder="Masukkan password baru"
                        class="w-full h-10 rounded-xl border border-gray-200 pl-4 pr-10 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    <button type="button" onclick="toggleVisibility('inputPasswordBaru', 'eyeBaru')"
                        class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                        <svg id="eyeBaru" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                    </button>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="password" id="inputPasswordKonfirmasi" placeholder="Ulangi password baru"
                        class="w-full h-10 rounded-xl border border-gray-200 pl-4 pr-10 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    <button type="button" onclick="toggleVisibility('inputPasswordKonfirmasi', 'eyeKonfirmasi')"
                        class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                        <svg id="eyeKonfirmasi" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                    </button>
                </div>
                <p id="passwordError" class="hidden mt-1 text-xs text-red-500">Password tidak cocok</p>
            </div>
...
function showAlert(message, type = 'success') {
    ...
    if (!box || !inner) { alert(message); return; }
...
async function fetchPegawai() {
    try {
        const res = await fetch('{{ route("admin.pengguna.all") }}', {
            headers: { 'Accept': 'application/json' }
        });
        cachedPegawai = await res.json();
        populateDropdown('', cachedPegawai);
    } catch (e) {
        console.error('Gagal fetch pegawai:', e);
    }
}
...
window.pilihPegawai = function (emp) {
    document.getElementById('searchPegawai').value = emp.nama;
    document.getElementById('dropdownPegawai').classList.add('hidden');
    window.location.href = '{{ route("admin.pengguna") }}?search=' + encodeURIComponent(emp.nama);
};
...
window.openCreateModal = function () {
    ['createNama','createNip','createNipBps','createEmail','createPassword','createPasswordKonfirmasi']
        .forEach(id => document.getElementById(id).value = '');
    document.getElementById('createGolongan').value = '';
    document.getElementById('createRole').value = '';
    document.getElementById('createPasswordError').classList.add('hidden');
...
window.submitCreatePegawai = async function () {
    const password   = document.getElementById('createPassword').value;
    const konfirmasi = document.getElementById('createPasswordKonfirmasi').value;
    const errorEl    = document.getElementById('createPasswordError');

    if (password !== konfirmasi) { errorEl.classList.remove('hidden'); return; }
    errorEl.classList.add('hidden');

    const payload = {
        nama:     document.getElementById('createNama').value.trim(),
        nip:      document.getElementById('createNip').value.trim(),
        nip_lama: document.getElementById('createNipBps').value.trim(),
        email:    document.getElementById('createEmail').value.trim(),
        golongan: document.getElementById('createGolongan').value,
        role:     document.getElementById('createRole').value,
        password,
    };

    if (!payload.nama || !payload.email || !payload.role || !payload.password) {
        showAlert('Field wajib belum diisi.', 'error');
        return;
    }

    try {
        const res = await fetch('{{ route("admin.pengguna.store") }}', {
            method: 'POST',
        ...
        if (res.ok) {
            showAlert(data.message ?? 'Pegawai berhasil ditambahkan');
            closeCreateModal();
...
document.getElementById('modalCreatePegawai')?.addEventListener('click', function (e) {
...
window.openPasswordModal = function (id, nama) {
    passwordTargetId = id;
    document.getElementById('passwordModalNama').textContent = nama;
...
window.submitPassword = async function () {
    ...
    if (baru !== konfirmasi) { errorEl.classList.remove('hidden'); return; }
    if (!baru) { showAlert('Password tidak boleh kosong.', 'error'); return; }
    errorEl.classList.add('hidden');

    try {
        const res = await fetch(`/admin/pengguna/${passwordTargetId}/password`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' },
        ...
        const data = await res.json();
        if (res.ok) {
            showAlert(data.message ?? 'Password berhasil diubah');
            closePasswordModal();
        } else {
...
document.querySelectorAll('tbody tr[data-id]').forEach(row => {
    ...
    btnEdit?.addEventListener('click', () => {
        row.querySelectorAll('.cell-edit').forEach(el => { el.dataset.original = el.value; });
    ...
    btnSave?.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) {
                row.querySelectorAll('.cell-edit').forEach(el => {
                    if (!el.name) return;
                    const display = el.closest('td').querySelector('.cell-display');
                    if (display) display.textContent = el.value || '—';
                });
                exitEdit(row, btnEdit, btnSave, btnCancel, btnDelete);
                showAlert(data.message ?? 'Pegawai berhasil diupdate');
            } else {
    ...
    btnCancel?.addEventListener('click', () => {
        row.querySelectorAll('.cell-edit').forEach(el => {
            if (el.dataset.original !== undefined) el.value = el.dataset.original;
        });
        exitEdit(row, btnEdit, btnSave, btnCancel, btnDelete);
    });

    btnDelete?.addEventListener('click', async () => {
        const nama = row.querySelector('.cell-display')?.textContent.trim();
        ...
        try {
            ...
            if (res.ok) {
                row.remove();
                showAlert(data.message ?? 'Pegawai berhasil dihapus');
            } else {
...
function exitEdit(row, btnEdit, btnSave, btnCancel, btnDelete) {
    row.querySelectorAll('.cell-display').forEach(el => el.classList.remove('hidden'));
    row.querySelectorAll('.cell-edit').forEach(el => el.classList.add('hidden'));
