        ...
        <table class="w-full min-w-[900px]">
            <thead>
                <tr class="bg-gray-100">
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Golongan</th>
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Uang Lembur Hari Kerja</th>
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Uang Lembur Hari Libur</th>
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Uang Makan</th>
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Pajak</th>
                    <th class="px-4 py-4 lg:p-5 text-center text-sm font-semibold text-gray-900">Aksi</th>
                </tr>
            </thead>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($rates as $r)
                    <tr class="bg-white hover:bg-gray-50 text-center"
                        ...
                        {{-- Golongan --}}
                        <td class="px-4 py-4 lg:p-5 text-sm font-medium text-gray-900 whitespace-nowrap">
                            {{ $r['golongan'] ?? '—' }}
                        </td>

                        {{-- Uang Lembur Hari Kerja --}}
                        <td class="px-4 py-4 lg:p-5 text-sm text-gray-900 whitespace-nowrap">
                            <span class="cell-display">
                                Rp {{ number_format($r['hari_kerja']->uang_lembur ?? 0, 0, ',', '.') }}
                            </span>
                            <input type="number" name="uang_lembur_kerja"
                                value="{{ $r['hari_kerja']->uang_lembur ?? 0 }}"
                                class="cell-edit hidden w-full min-w-[130px] rounded-lg border border-gray-200 px-2 py-1 text-center text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        </td>

                        {{-- Uang Lembur Hari Libur --}}
                        <td class="px-4 py-4 lg:p-5 text-sm text-gray-900 whitespace-nowrap">
                            <span class="cell-display">
                                Rp {{ number_format($r['hari_libur']->uang_lembur ?? 0, 0, ',', '.') }}
                            </span>
                            <input type="number" name="uang_lembur_libur"
                                value="{{ $r['hari_libur']->uang_lembur ?? 0 }}"
                                class="cell-edit hidden w-full min-w-[130px] rounded-lg border border-gray-200 px-2 py-1 text-center text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        </td>
                        ...
                        <td class="px-4 py-4 lg:p-5 text-sm text-gray-900 whitespace-nowrap">
                            <span class="cell-display">
                                Rp {{ number_format($r['hari_kerja']->uang_makan ?? 0, 0, ',', '.') }}
                            </span>
                            <input type="number" name="uang_makan"
                                value="{{ $r['hari_kerja']->uang_makan ?? 0 }}"
                                class="cell-edit hidden w-full min-w-[130px] rounded-lg border border-gray-200 px-2 py-1 text-center text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        </td>
                        ...
                        <td class="px-4 py-4 lg:p-5 text-sm text-gray-900 whitespace-nowrap">
                            <span class="cell-display">
                                {{ ($r['hari_kerja']->pajak ?? 0) * 100 }}%
                            </span>
                            <input type="number" step="0.01" name="pajak"
                                value="{{ $r['hari_kerja']->pajak ?? 0 }}"
                                class="cell-edit hidden w-full min-w-[90px] rounded-lg border border-gray-200 px-2 py-1 text-center text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                        </td>

                        {{-- Aksi --}}
                        <td class="px-4 py-4 lg:p-5 text-sm text-gray-900">
                            <div class="flex items-center justify-center gap-1">

                                {{-- Edit --}}
                                <button type="button"
                                    class="btn-edit p-2 rounded-full hover:bg-indigo-50 transition-all flex items-center">
                                ...
                                {{-- Simpan --}}
                                <button type="button"
                                    class="btn-save hidden p-2 rounded-full hover:bg-green-50 transition-all flex items-center">
                                ...
                                {{-- Batal --}}
                                <button type="button"
                                    class="btn-cancel hidden p-2 rounded-full hover:bg-red-50 transition-all flex items-center">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="p-10 text-center text-sm text-gray-400">
...
function formatPajak(value) {
    return (Number(value || 0) * 100) + '%';
}

document.querySelectorAll('tbody tr[data-kerja-id]').forEach(row => {
    ...
    btnSave?.addEventListener('click', async () => {
        ...
        if (!kerjaId || !liburId) {
            showAlert('ID tarif tidak ditemukan.', 'error');
            return;
        }
        ...
        try {
            ...
            if (resKerja.ok && resLibur.ok) {
                row.querySelector('[name="uang_lembur_kerja"]')
                    .closest('td')
                    .querySelector('.cell-display')
                    .textContent = formatRupiah(uangLemburKerja);

                row.querySelector('[name="uang_lembur_libur"]')
                    .closest('td')
                    .querySelector('.cell-display')
                    .textContent = formatRupiah(uangLemburLibur);

                row.querySelector('[name="uang_makan"]')
                    .closest('td')
                    .querySelector('.cell-display')
                    .textContent = formatRupiah(uangMakan);

                row.querySelector('[name="pajak"]')
                    .closest('td')
                    .querySelector('.cell-display')
                    .textContent = formatPajak(pajak);
