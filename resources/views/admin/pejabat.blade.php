...
<div class="max-w-7xl mx-auto px-8 my-5">
    ...
    <div id="alertContainer" class="mb-4">
        @if(session('success'))
            <div class="alert-inline px-4 py-3 rounded-lg border border-green-200 bg-green-100 text-sm text-green-700">
                {{ session('success') }}
            </div>
        @endif

        @if(session('error'))
            <div class="alert-inline px-4 py-3 rounded-lg border border-red-200 bg-red-100 text-sm text-red-700">
                {{ session('error') }}
            </div>
        @endif
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full rounded-xl table-auto">
            <thead>
                <tr class="bg-gray-50">
                    <th class="px-3 py-2 text-xs font-semibold text-gray-900 rounded-tl-xl">Tahun</th>
                    <th class="px-3 py-2 text-xs font-semibold text-gray-900">Nama PPK</th>
            ...
            <tbody class="divide-y divide-gray-300">
                ...
                <tr class="bg-white hover:bg-gray-50" data-tahun="{{ $tahun }}">
                    {{-- Tahun --}}
                    <td class="px-3 py-3 text-xs text-gray-900 font-semibold">{{ $tahun }}</td>
                    ...
                    <td class="px-3 py-3 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <button class="btn-edit p-2 rounded-full hover:bg-indigo-50 transition-all">
                                {{-- icon pensil --}}
                            ...
                            <button class="btn-delete p-2 rounded-full hover:bg-red-50 transition-all">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button class="btn-save hidden p-2 rounded-full hover:bg-green-50 transition-all">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                </svg>
                            </button>
                            <button class="btn-cancel hidden p-2 rounded-full hover:bg-red-50 transition-all">
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-3 py-8 text-center text-sm text-gray-400">Belum ada data pejabat.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="relative flex min-h-screen items-center justify-center p-4">
        <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl">
            <div class="flex items-center justify-between border-b px-6 py-4">
                <h2 class="text-lg font-semibold text-gray-900">Tambah Pejabat</h2>
                <button onclick="closeModal()" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
            </div>
            <form action="{{ route('admin.pejabat.store') }}" method="POST" class="px-6 py-5 space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tahun</label>
                    <input type="number" name="tahun" min="2020" max="2099" required
                        placeholder="2026"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">NIP Lama</label>
                    <input type="text" name="nip_lama"
                        class="w-full rounded-md border border-gray-300 px-4 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">NIP Baru</label>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit"
                        class="px-4 py-2 text-sm font-semibold text-black bg-[#faa938] rounded-lg hover:bg-[#fd9a10] hover:text-white">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="modalEdit" class="fixed inset-0 z-50 hidden">
    ...
    <div class="relative flex min-h-screen items-center justify-center p-4">
        <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl">
            <div class="flex items-center justify-between border-b px-6 py-4">
                <h2 class="text-lg font-semibold text-gray-900">Edit Pejabat <span id="editTahunLabel"></span></h2>
                <button onclick="closeModalEdit()" class="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
            </div>
            <div class="px-6 py-5 space-y-5">
                    ...
                    <form id="formEditPpk" method="POST" class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div>
                                ...
                                <select name="status" id="ppkStatus"
                                    class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900">
                                    <option value="aktif">Aktif</option>
                                    <option value="nonaktif">Nonaktif</option>
                                </select>
                            </div>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="px-4 py-2 text-sm font-semibold text-black bg-[#faa938] rounded-lg hover:bg-[#fd9a10] hover:text-white">
                                Simpan PPK
                            </button>
                        </div>
                    </form>
                ...
                {{-- KBU --}}
                <div class="border rounded-xl p-4 space-y-3">
                    <p class="text-sm font-semibold text-gray-700">Kepala Bagian Umum</p>
                    <form id="formEditKbu" method="POST" class="space-y-3">
                        @csrf @method('PUT')
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">Nama</label>
                            <input type="text" name="nama" id="kbuNama"
                                class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900">
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">NIP Lama</label>
                                <input type="text" name="nip_lama" id="kbuNipLama"
                                    class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Status</label>
                                <select name="status" id="kbuStatus"
                                    class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-gray-900">
                                    <option value="aktif">Aktif</option>
                                    <option value="nonaktif">Nonaktif</option>
                                </select>
...
document.querySelectorAll('tbody tr[data-tahun]').forEach(row => {
    ...
    btnEdit?.addEventListener('click', () => {
        ...
        row.querySelectorAll('[class*="cell-edit"]').forEach(el => el.classList.remove('hidden'));
