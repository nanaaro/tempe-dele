            ...
            <tr class="bg-white hover:bg-gray-50" data-kode="{{ $t->kode_tim }}">
                ...
                <td class="p-5 text-sm text-gray-900">
                    ...
                    <input type="text" name="nama_tim" value="{{ $t->nama_tim }}"
                        class="text-right cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                </td>
                ...
                {{-- Penugasan Khusus --}}
                <td class="text-center p-5 text-sm text-gray-900">
                    <span class="cell-display">{{ $t->is_penugasan_khusus == 1 ? 'Ya' : 'Tidak' }}</span>
                    <select name="is_penugasan_khusus"
                        class="text-center cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20">
                        <option value="0" {{ !$t->is_penugasan_khusus ? 'selected' : '' }}>Tidak</option>
                        <option value="1" {{ $t->is_penugasan_khusus ? 'selected' : '' }}>Ya</option>
                    </select>
                </td>

                {{-- Jumlah Anggota (read-only) --}}
                <td class="text-center p-5 text-sm text-gray-900">
                    <span>{{ $t->jumlah_anggota ?? 0 }}</span>
                </td>

                {{-- Status --}}
                <td class="text-center p-5 text-sm text-gray-900 w-32">
                ...
                <td class="text-center p-5 text-sm text-gray-900">
                    <div class="flex items-center justify-center gap-1">
                        ...
                        {{-- Lihat Detail --}}
                        <button onclick="openModalAnggota('{{ $t->kode_tim }}', '{{ $t->nama_tim }}')"
                        <a href="/admin/tim/{{ $t->kode_tim }}"
                        ...
                        {{-- Simpan (hidden) --}}
                        <button class="btn-save hidden p-2 rounded-full transition-all duration-500 flex items-center hover:bg-green-50">
                        ...
                        {{-- Batal (hidden) --}}
                        <button class="btn-cancel hidden p-2 rounded-full transition-all duration-500 flex items-center hover:bg-red-50">
    ...
    <nav class="inline-flex items-center p-1 rounded bg-white space-x-2">
        ...
        @else
            <a class="p-1 rounded border text-black bg-white hover:text-white hover:bg-[#faa938] hover:border-[#faa938]"
               href="{{ $tim->previousPageUrl() }}&search={{ request('search') }}">
                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z"/>
                </svg>
            </a>
        ...
        <p class="text-gray-500 text-sm">Page {{ $tim->currentPage() }} of {{ $tim->lastPage() }}</p>

        {{-- Next --}}
        @if ($tim->hasMorePages())
            <a class="p-1 rounded border text-black bg-white hover:text-white hover:bg-[#faa938] hover:border-[#faa938]"
               href="{{ $tim->nextPageUrl() }}&search={{ request('search') }}">
                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
...
<div id="createModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-4">Tambah Tim Baru</h2>
        <div class="space-y-3">
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Kode Tim <span class="text-red-400">*</span></label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Nama Tim <span class="text-red-400">*</span></label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">NIP Lama Ketua</label>
                    <input type="text" id="new_niplama_ketua"
                        class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">NIP Baru Ketua</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Penugasan Khusus</label>
            ...
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1">Tanggal Non-aktif</label>
        ...
        <div class="flex justify-end gap-2 mt-5">
            <button onclick="closeCreateModal()"
                class="px-4 py-2 rounded-lg text-sm text-gray-600 border border-gray-200 hover:bg-gray-50 transition-all">
                Batal
            </button>
            <button onclick="submitCreateTim()"
                class="px-4 py-2 rounded-lg text-sm text-white bg-[#faa938] hover:brightness-95 transition-all">
                Simpan
            </button>
        </div>
    </div>
...
{{-- Modal Anggota Tim --}}
<div id="modalAnggota" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/40">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[90vh] flex flex-col">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <button onclick="closeModalAnggota()" class="p-2 rounded-full hover:bg-gray-100 transition-all">
        ...
        {{-- PANEL 1: Daftar Anggota --}}
        <div id="panelAnggota" class="flex flex-col flex-1 overflow-hidden">

            {{-- Search + Tombol Tambah --}}
            <div class="flex items-center gap-2 mb-3">
                <input type="text" id="searchAnggota" placeholder="Cari nama anggota..."
                    oninput="filterAnggota()"
                    class="flex-1 h-9 rounded-xl border border-gray-200 bg-white pl-4 pr-3 text-sm text-gray-700 focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                <button onclick="showPanelTambah()"
                    class="flex items-center gap-1.5 h-9 px-3 rounded-xl bg-[#faa938] text-white text-sm hover:brightness-95 transition-all">
            ...
            {{-- Tabel Anggota --}}
            <div class="overflow-y-auto flex-1 rounded-xl border border-gray-100">
            ...
            {{-- Pagination Anggota --}}
            <div id="paginasiAnggota" class="flex items-center justify-center gap-2 mt-3"></div>
        </div>

        {{-- PANEL 2: Tambah Anggota --}}
        <div id="panelTambah" class="hidden flex-col flex-1 overflow-hidden">

            {{-- Back + Search --}}
            <div class="flex items-center gap-2 mb-3">
                <button onclick="showPanelAnggota()" class="p-2 rounded-full hover:bg-gray-100 transition-all">
            ...
            {{-- Pagination Pegawai --}}
            <div id="paginasiPegawai" class="flex items-center justify-center gap-2 mt-3"></div>
        </div>
...
async function fetchTim() {
    try {
        ...
        cachedTim = data.data ?? data; // support paginated atau array biasa
...
function populateDropdownTim(filter = '', data = []) {
    ...
    const filtered = data.filter(t =>
        t.nama_tim?.toLowerCase().includes(filter.toLowerCase())
    );
...
window.filterDropdownTim = function () {
    ...
    // Kalau input dikosongkan, reset tabel
    if (search === '') {
...
window.pilihTim = function (tim) {
    ...
    // Redirect dengan search query → filter tabel server-side
...
document.querySelectorAll('tbody tr[data-kode]').forEach(row => {
    ...
    btnSave?.addEventListener('click', async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                // Update tampilan cell-display
                row.querySelectorAll('.cell-edit').forEach(el => {
                    if (!el.name) return;
                    const display = el.closest('td').querySelector('.cell-display');
                    if (!display) return;
                    if (el.name === 'is_penugasan_khusus') {
                        display.textContent = el.value == '1' ? 'Ya' : '—';
                    } else if (el.name === 'status') {
                        display.textContent = el.value === 'aktif' ? 'Aktif' : 'Non-aktif';
                    } else if (el.name === 'tanggal_non_aktif') {
                        display.textContent = el.value
                            ? new Date(el.value).toLocaleDateString('id-ID') : '—';
                    } else {
                        display.textContent = el.value || '—';
                    }
                });
...
let anggotaPage = 1;
let pegawaiPage = 1;
let searchAnggotaTimer, searchPegawaiTimer;

window.openModalAnggota = function(kodeTim, namaTim) {
    currentKodeTim = kodeTim;
    document.getElementById('modalAnggotaTitle').textContent = namaTim;
...
window.closeModalAnggota = function() {
    ...
    document.getElementById('modalAnggota').classList.remove('flex');
    ...
    if (anggota.length === 0) {
        ...
        tbody.innerHTML = anggota.map(a => `
            <tr class="hover:bg-gray-50">
                <td class="p-4 text-sm text-gray-800">${a.nama ?? '—'}</td>
                <td class="p-4 text-sm text-gray-500">${a.nip ?? '—'}</td>
                <td class="p-4 text-center">
                    <button onclick="hapusAnggota(${a.id_pegawai}, '${a.nama}')"
                        class="p-1.5 rounded-full hover:bg-red-50 transition-all">
                        <svg width='16' height='16' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg' stroke='red-600'>
...
async function loadPegawaiTersedia(page = 1) {
    ...
    if (pegawai.length === 0) {
        ...
        tbody.innerHTML = pegawai.map(p => `
            <tr class="hover:bg-gray-50">
                <td class="p-4 text-sm text-gray-800">${p.nama ?? '—'}</td>
                <td class="p-4 text-sm text.gray-500">${p.nip ?? '—'}</td>
                <td class="p-4 text-center">
                    <button onclick="tambahAnggota(${p.id_pegawai}, '${p.nama}')"
                        class="px-3 py-1 rounded-lg text-xs bg-[#faa938] text-white hover:brightness-95 transition-all">
                        Tambah
                    </button>
                </td>
            </tr>
...
function renderPaginasi(elId, data, loadFn) {
    ...
    el.innerHTML = `
        <button onclick="${loadFn.name}(${data.current_page - 1})"
            ${data.current_page === 1 ? 'disabled' : ''}
            class="p-1 rounded border text-sm ${data.current_page === 1 ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-[#faa938] hover:text-white hover:border-[#faa938]'}">
            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                <path fill-rule="evenodd" d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z"/>
            </svg>
        </button>
        <span class="text-xs text-gray-500">Page ${data.current_page} of ${data.last_page}</span>
        <button onclick="${loadFn.name}(${data.current_page + 1})"
            ${data.current_page === data.last_page ? 'disabled' : ''}
            class="p-1 rounded border text-sm ${data.current_page === data.last_page ? 'text-gray-300 cursor-not-allowed' : 'hover:bg-[#faa938] hover:text-white hover:border-[#faa938]'}">
            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                <path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/>
            </svg>
        </button>
