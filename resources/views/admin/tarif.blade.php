...
@section('title', 'Akumulasi Tarif Lembur')
...
<div class="max-w-7xl mx-auto px-8 my-5">
    ...
    {{-- Alert --}}
    <div id="alertBox" class="hidden mb-3">
        ...
        <table class="min-w-full">
            <thead>
                <tr class="bg-gray-50">
                    ...
                    <th class="p-5 text-center text-sm font-semibold text-gray-900">Uang Lembur Hari Kerja</th>
                    <th class="p-5 text-center text-sm font-semibold text-gray-900">Uang Lembur Hari Libur</th>
                    <th class="p-5 text-center text-sm font-semibold text-gray-900">Uang Makan</th>
                    <th class="p-5 text-center text-sm font-semibold text-gray-900">Pajak</th>
                    <th class="p-5 text-center text-sm font-semibold text-gray-900">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="bg-white hover:bg-gray-50 text-center"
                    data-kerja-id="{{ $r['hari_kerja']->id_rate ?? '' }}"
                    data-libur-id="{{ $r['hari_libur']->id_rate ?? '' }}">
                    ...
                    <td class="p-5 text-sm text-gray-900">
                        <span class="cell-display">Rp {{ number_format($r['hari_kerja']->uang_lembur ?? 0, 0, ',', '.') }}</span>
                        <input type="number" name="uang_lembur_kerja" value="{{ $r['hari_kerja']->uang_lembur ?? 0 }}"
                            class="cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm text-center focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    </td>

                    {{-- Uang Lembur Hari Libur --}}
                    <td class="p-5 text-sm text-gray-900">
                        <span class="cell-display">Rp {{ number_format($r['hari_libur']->uang_lembur ?? 0, 0, ',', '.') }}</span>
                        <input type="number" name="uang_lembur_libur" value="{{ $r['hari_libur']->uang_lembur ?? 0 }}"
                            class="cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm text-center focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    </td>

                    {{-- Uang Makan (sama untuk kerja & libur, ambil dari hari kerja) --}}
                    <td class="p-5 text-sm text-gray-900">
                        <span class="cell-display">Rp {{ number_format($r['hari_kerja']->uang_makan ?? 0, 0, ',', '.') }}</span>
                        <input type="number" name="uang_makan" value="{{ $r['hari_kerja']->uang_makan ?? 0 }}"
                            class="cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm text-center focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    </td>

                    {{-- Pajak --}}
                    <td class="p-5 text-sm text-gray-900">
                        <span class="cell-display">{{ ($r['hari_kerja']->pajak ?? 0) * 100 }}%</span>
                        <input type="number" step="0.01" name="pajak" value="{{ $r['hari_kerja']->pajak ?? 0 }}"
                            class="cell-edit hidden w-full rounded-lg border border-gray-200 px-2 py-1 text-sm text-center focus:border-[#faa938] focus:outline-none focus:ring-2 focus:ring-[#faa938]/20"/>
                    </td>
                    ...
                    <td class="p-5 text-sm text-gray-900">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            {{-- Edit --}}
                            <button class="btn-edit p-2 rounded-full hover:bg-indigo-50 transition-all flex items-center">
                ...
                @empty
                <tr>
                    <td colspan="6" class="p-10 text-center text-sm text-gray-400">Belum ada data tarif.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    ...
    document.querySelectorAll('tbody tr').forEach(row => {
        const kerjaId = row.dataset.kerjaId;
        const liburId = row.dataset.liburId;
        ...
        btnSave?.addEventListener('click', async () => {
            const uangLemburKerja = row.querySelector('[name="uang_lembur_kerja"]').value;
            const uangLemburLibur = row.querySelector('[name="uang_lembur_libur"]').value;
            const uangMakan       = row.querySelector('[name="uang_makan"]').value;
            const pajak           = row.querySelector('[name="pajak"]').value;

            try {
                // Update hari kerja
                const resKerja = await fetch(`/admin/tarif/${kerjaId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' },
                    body: JSON.stringify({ uang_lembur: uangLemburKerja, uang_makan: uangMakan, pajak }),
                });

                // Update hari libur
                const resLibur = await fetch(`/admin/tarif/${liburId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' },
                    body: JSON.stringify({ uang_lembur: uangLemburLibur, uang_makan: uangMakan, pajak }),
                });

                if (resKerja.ok && resLibur.ok) {
                    // Update display
                    row.querySelector('[name="uang_lembur_kerja"]').closest('td').querySelector('.cell-display').textContent
                        = 'Rp ' + Number(uangLemburKerja).toLocaleString('id-ID');
                    row.querySelector('[name="uang_lembur_libur"]').closest('td').querySelector('.cell-display').textContent
                        = 'Rp ' + Number(uangLemburLibur).toLocaleString('id-ID');
                    row.querySelector('[name="uang_makan"]').closest('td').querySelector('.cell-display').textContent
                        = 'Rp ' + Number(uangMakan).toLocaleString('id-ID');
                    row.querySelector('[name="pajak"]').closest('td').querySelector('.cell-display').textContent
                        = (pajak * 100) + '%';

                    exitEdit(row, btnEdit, btnSave, btnCancel);
                    showAlert('Tarif berhasil diupdate');
                } else {
    ...
    function exitEdit(row, btnEdit, btnSave, btnCancel) {
