                ...
                <table class="min-w-full rounded-xl table-auto">
                    ...
                    <tbody class="divide-y divide-gray-300">
                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                            ...
                            <td class="px-3 py-3 text-center">
                                <span id="status-badge-1" class="bg-amber-100 rounded-full px-3 text-xs text-amber-700 py-0.5">Menunggu</span>
                            </td>
                        </tr>
                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                            <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900">Pegawai 2</td>
                            <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900 text-center">67890</td>
                            <td class="px-3 py-3 text-center">
                                <span id="presensi-badge-2">
                                    <button onclick="openModal(2, 'tepat')" class="text-xs font-semibold text-slate-500 hover:underline">Validasi</button>
                                </span>
                            </td>
                            <td class="px-3 py-3 text-sm font-medium text-gray-900 text-center">16.00 - 18.00</td>
                            <td class="px-3 py-3 text-sm font-medium text-gray-900 text-center">Input data</td>
                            <td class="px-3 py-3 text-center">
                                <span id="keputusan-btn-2">
                                    <button onclick="openModalKeputusan(2)" class="text-xs font-semibold text-slate-500 hover:underline">Tinjau</button>
                                </span>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <span id="status-badge-2" class="bg-amber-100 rounded-full px-3 text-xs text-amber-700 py-0.5">Menunggu</span>
                            </td>
                        </tr>
                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                            <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900">Pegawai 3</td>
                            <td class="px-3 py-3 whitespace-nowrap text-sm font-medium text-gray-900 text-center">11223</td>
                            <td class="px-3 py-3 text-center">
                                <span id="presensi-badge-3">
                                    <button onclick="openModal(3, 'tepat')" class="text-xs font-semibold text-slate-500 hover:underline">Validasi</button>
                                </span>
                            </td>
                            <td class="px-3 py-3 text-sm font-medium text-gray-900 text-center">17.00 - 19.00</td>
                            <td class="px-3 py-3 text-sm font-medium text-gray-900 text-center">Input data</td>
                            <td class="px-3 py-3 text-center">
                                <span id="keputusan-btn-3">
                                    <button onclick="openModalKeputusan(3)" class="text-xs font-semibold text-slate-500 hover:underline">Tinjau</button>
                                </span>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <span id="status-badge-3" class="bg-amber-100 rounded-full px-3 text-xs text-amber-700 py-0.5">Menunggu</span>
                            </td>
                        </tr>
                    </tbody>
                </table>
...
<script>
    const rowState = {};
    let keputusanAkhir = null;
    let keputusan = null;
    let currentRow = null;

    function openModal(rowId, skenario) {
        keputusan = null;
        currentRow = rowId;
        resetKeputusan();

        document.getElementById('roStatus').textContent = 'WFO';
        document.getElementById('roJamDatang').textContent = skenario === 'tepat' ? '08.00' : '09.15';
    ...
    function setKeputusan(val) {
        ...
        resetKeputusan();
        if (val === 'tolak') {
            document.getElementById('btnTolak').className = 'px-4 py-2 text-sm font-semibold rounded-lg border border-red-400 bg-red-50 text-red-600 transition-all';
        } else {
            document.getElementById('btnSetujui').className = 'px-4 py-2 text-sm font-semibold rounded-lg border border-green-400 bg-green-50 text-green-700 transition-all';
        }
    }

    function resetKeputusan() {
        document.getElementById('btnTolak').className = 'px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-600 hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition-all';
        document.getElementById('btnSetujui').className = 'px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-all';
    }

    function simpan() {
        if (!keputusan) { alert('Pilih keputusan terlebih dahulu.'); return; }

        rowState[currentRow] = { validasi: keputusan };

        // Update badge presensi
        const presensiBadge = document.getElementById(`presensi-badge-${currentRow}`);
        if (keputusan === 'setujui') {
            presensiBadge.innerHTML = `<span class="px-3 text-xs font-semibold text-green-700 py-0.5">Disetujui</span>`;
        } else {
            presensiBadge.innerHTML = `<span class="bg-red-100 rounded-full px-3 text-xs text-red-600 py-0.5">Ditolak</span>`;
            updateKeputusanBadge(currentRow, 'tolak');
            updateStatusBadge(currentRow, 'tolak');
        }

        closeModal();
    }

    function openModalKeputusan(rowId) {
        currentRow = rowId;
        const state = rowState[rowId];
        keputusanAkhir = null;

        document.getElementById('kJamMulai').value = '';
        document.getElementById('kJamSelesai').value = '';
        document.getElementById('kCatatan').value = '';

        const kBtnSetujui = document.getElementById('kBtnSetujui');
        const kBtnTolak = document.getElementById('kBtnTolak');

        kBtnSetujui.disabled = false;
        resetBtnKeputusan();

        if (state && state.validasi === 'tolak') {
            keputusanAkhir = 'tolak';
            kBtnSetujui.disabled = true;
            kBtnSetujui.className = 'px-4 py-2 text-sm font-medium border border-gray-200 rounded-lg text-gray-300 cursor-not-allowed';
            kBtnTolak.className = 'px-4 py-2 text-sm font-semibold rounded-lg border border-red-400 bg-red-50 text-red-600 transition-all';
        }
    ...
    function setKeputusanAkhir(val) {
        if (rowState[currentRow]?.validasi === 'tolak' && val === 'setujui') return;
        keputusanAkhir = val;
        resetBtnKeputusan();
        if (val === 'tolak') {
    ...
    function resetBtnKeputusan() {
        ...
        const btnSetujui = document.getElementById('kBtnSetujui');
        if (!btnSetujui.disabled) {
            btnSetujui.className = 'px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg text-gray-600 hover:bg-green-50 hover:border-green-300 hover:text-green-700 transition-all';
        }
    }

    function simpanKeputusan() {
        if (!keputusanAkhir) { alert('Pilih keputusan terlebih dahulu.'); return; }
        updateKeputusanBadge(currentRow, keputusanAkhir);
        updateStatusBadge(currentRow, keputusanAkhir);
        closeModalKeputusan();
    }

    function updateKeputusanBadge(rowId, hasil) {
        const span = document.getElementById(`keputusan-btn-${rowId}`);
        if (hasil === 'setujui') {
            span.innerHTML = `<span class="bg-green-100 rounded-full px-3 text-xs text-green-700 py-0.5">Disetujui</span>`;
        } else {
            span.innerHTML = `<span class="bg-red-100 rounded-full px-3 text-xs text-red-600 py-0.5">Ditolak</span>`;
        }
    }

    function updateStatusBadge(rowId, hasil) {
        const badge = document.getElementById(`status-badge-${rowId}`);
        if (hasil === 'setujui') {
            badge.className = 'bg-green-100 rounded-full px-3 text-xs text-green-700 py-0.5';
            badge.textContent = 'Disetujui';
        } else {
            badge.className = 'bg-red-100 rounded-full px-3 text-xs text-red-600 py-0.5';
