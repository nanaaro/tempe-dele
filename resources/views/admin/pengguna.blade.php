...
<div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 my-4 sm:my-5">
    <div class="flex flex-col sm:flex-row sm:items-center gap-3">
        ...
        {{-- Filter Pegawai --}}
        <div class="relative w-full sm:flex-1 sm:min-w-[260px]" id="pegawaiPicker">
        ...
        {{-- Sinkron --}}
        <button type="button" title="Sinkron Pegawai dari API" onclick="sinkronPegawai()"
            ...
            <span class="sm:hidden text-sm font-medium">Sinkron Pegawai</span>
        </button>

        {{-- Tambah Pegawai --}}
        <button type="button" title="Tambah Pegawai" onclick="openCreateModal()"
            ...
            <span class="sm:hidden text-sm font-medium">Tambah Pegawai</span>
        </button>
...
{{-- ===================== ALERT ===================== --}}
<div id="alertBox" class="hidden max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 mb-3">
    <div id="alertInner" class="rounded-xl px-4 py-3 text-sm font-medium"></div>
</div>

{{-- ===================== TABEL ===================== --}}
<div class="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 my-5">
    @php
        ...
        <table class="min-w-[1180px] lg:min-w-full table-auto">
            <thead>
                <tr class="bg-gray-100">
                    ...
                    <th class="p-3 sm:p-5 text-center text-xs sm:text-sm font-semibold text-gray-900 rounded-tr-xl whitespace-nowrap">
                        Aksi
                    </th>
                </tr>
            </thead>
            ...
            <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="bg-white hover:bg-gray-50 text-center" data-id="{{ $p->id_pegawai }}">
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 text-left">
                            ...
                            <input type="text" name="nama" value="{{ $p->nama }}"
                                class="cell-edit hidden w-full min-w-[180px] border border-gray-200 rounded-lg px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none"/>
                        </td>
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 whitespace-nowrap">
                            ...
                            <input type="text" name="nip" value="{{ $p->nip }}"
                                class="cell-edit hidden w-full min-w-[150px] border border-gray-200 rounded-lg px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none"/>
                        </td>
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 whitespace-nowrap">
                            ...
                            <input type="text" name="nip_lama" value="{{ $p->nip_lama }}"
                                class="cell-edit hidden w-full min-w-[120px] border border-gray-200 rounded-lg px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none"/>
                        </td>
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 whitespace-nowrap">
                            ...
                            <button type="button"
                                class="cell-edit hidden text-xs text-[#faa938] hover:underline whitespace-nowrap"
                                onclick="openPasswordModal({{ $p->id_pegawai }}, '{{ addslashes($p->nama) }}')">
                                Ganti password
                            </button>
                        </td>
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 whitespace-nowrap">
                            ...
                            <select name="golongan"
                                class="cell-edit hidden w-full min-w-[90px] border border-gray-200 rounded-lg px-2 py-1 text-sm focus:border-[#faa938] focus:outline-none">
                                @foreach(['I','II','III','IV'] as $gol)
                                    <option value="{{ $gol }}" {{ $p->golongan === $gol ? 'selected' : '' }}>
                                        {{ $gol }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <td class="p-3 sm:p-5 text-xs sm:text-sm text-gray-900 whitespace-nowrap">
                           <div class="flex items-center justify-center gap-2">
                                ...
                                <button type="button"
                                    class="btn-save hidden p-2 rounded-full hover:bg-green-50 transition-all text-green-500"
                                    title="Simpan">
                                ...
                                <button type="button"
                                    class="btn-cancel hidden p-2 rounded-full hover:bg-red-50 transition-all text-gray-400"
                                    title="Batal">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @empty
                    <tr>
                        <td colspan="8" class="p-10 text-center text-sm text-gray-400">
        ...
        <nav class="inline-flex items-center p-1 rounded bg-white space-x-2">
            ...
            @else
                <a class="p-1 rounded border text-black bg-white hover:text-white hover:bg-[#faa938] hover:border-[#faa938]"
                    href="{{ $pegawai->appends(request()->only('search'))->previousPageUrl() }}">
                    <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M11.354 1.646a.5.5 0 0 1 0 .708L5.707 8l5.647 5.646a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1 0-.708l6-6a.5.5 0 0 1 .708 0z"/>
                    </svg>
                </a>
            ...
            <p class="text-gray-500 text-xs sm:text-sm whitespace-nowrap">
                Page {{ $pegawai->currentPage() }} of {{ $pegawai->lastPage() }}
            </p>

            @if ($pegawai->hasMorePages())
                <a class="p-1 rounded border text-black bg-white hover:text-white hover:bg-[#faa938] hover:border-[#faa938]"
                    href="{{ $pegawai->appends(request()->only('search'))->nextPageUrl() }}">
                    <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
...
<div id="modalCreatePegawai" class="fixed inset-0 z-50 hidden">
        ...
        <div class="w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            <div class="px-4 sm:px-6 py-5 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" onclick="toggleVisibility('createPassword')"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" onclick="toggleVisibility('createPasswordKonfirmasi')"
                                class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        ...
                        <p id="createPasswordError" class="hidden mt-1 text-xs text-red-500">
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 pt-2">
                    <button type="button" onclick="closeCreateModal()"
                        class="w-full sm:w-auto px-4 py-2 text-sm font-medium rounded-xl border border-gray-200 text-gray-600 hover:border-gray-300 transition-colors">
                        Batal
                    </button>

                    <button type="button" onclick="submitCreatePegawai()"
                        class="w-full sm:w-auto px-4 py-2 text-sm font-medium rounded-xl bg-[#faa938] text-white hover:brightness-95 transition-all">
                        Simpan
                    </button>
                </div>
            </div>
...
<div id="modalPassword" class="fixed inset-0 z-50 hidden">
        ...
        <div class="w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            <div class="px-4 sm:px-6 py-5 space-y-4">
                <p class="text-xs text-gray-500">
                    Pegawai:
                    <span id="passwordModalNama" class="font-medium text-gray-800"></span>
                </p>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="toggleVisibility('inputPasswordBaru')"
                            class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="toggleVisibility('inputPasswordKonfirmasi')"
                            class="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (e) {
        const wrapperPegawai = document.getElementById('pegawaiPicker');
...
window.openCreateModal = function () {
    ...
    document.getElementById('modalCreatePegawai').classList.remove('hidden');
    document.body.classList.add('overflow-hidden');
};

window.closeCreateModal = function () {
    document.getElementById('modalCreatePegawai').classList.add('hidden');
...
// SINKRON PEGAWAI
// =====================
function sinkronPegawai() {
    if (!confirm('Sinkronisasi akan update data semua pegawai. Lanjutkan?')) return;

    fetch('{{ route("admin.pengguna.sinkron") }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': CSRF_TOKEN,
            'Content-Type': 'application/json',
    ...
    .catch(() => alert('Terjadi kesalahan saat sinkronisasi.'));
}
</script>
